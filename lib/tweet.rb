...
#methode de l'exo 1, inactive
def my_first_tweet_ruby(client)
...
def random_select(array, number, client)
 return array.sample(number)
end

def send_twitt_journalists(array, number, client)
  bonjour_monde = "\#bonjour_monde"
  good_work = "I love your last tweet !"
  thp = "@the_hacking_pro"
  random_select(array, number, client).each do |journalist|
  client.update(journalist + " " + good_work + " " + thp + " " + bonjour_monde)
  end
end

send_twitt_journalists(journalists, 5, client)
